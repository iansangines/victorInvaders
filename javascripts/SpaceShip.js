...
SI.EnemyRocket = function (options) {

	var defaultOptions = {
		x: 0,
		y: 0,
		direction: null,
		img: SI.Images.enemyRocketImg,
		exploded: false
	}
	for (var key in defaultOptions) {
		if(options.hasOwnProperty(key)) {
 ...
 * deltaY - amount to move on y-axis
 */
SI.EnemyRocket.prototype.move = function (deltaY) {
	this.y += this.direction * deltaY;
	if(this.y <= 0 || this.y + SI.Sizes.enemyrocketHeight >= SI.Sizes.bottomMargin) {
		this.exploded = true;
	}
}
